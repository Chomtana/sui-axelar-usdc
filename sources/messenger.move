...
module axelar::messenger {
    ...
    fun validate_proof(
        ...
        while (i < total_signatures) {
            ...
            let signed_by: vector<u8> = ecdsa::secp256k1_ecrecover(&signature, message, 0);
    ...
    fun test_ecrecover() {
        ...
        let pub_key = ecdsa::secp256k1_ecrecover(&signature, &to_signed(message), 0);
    ...
    fun operators_hash(operators: &vector<vector<u8>>): vector<u8> {
        sui::hash::keccak256(&bcs::to_bytes(operators))
    }
}